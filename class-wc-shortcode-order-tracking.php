...
class WC_Shortcode_Order_Tracking {
	...
	public static function output( $atts ) {
		...
		if ( isset( $_REQUEST['orderid'] ) && wp_verify_nonce( $nonce_value, 'woocommerce-order_tracking' ) ) { // WPCS: input var ok.
			...
			$order_phone = empty( $_REQUEST['order_phone'] ) ? '' : wc_sanitize_phone_number( wp_unslash( $_REQUEST['order_phone'] ) ); // WPCS: input var ok.
			if ( ! $order_id ) {
				wc_print_notice( __( 'Please enter a valid order ID', 'woocommerce' ), 'error' );
			} elseif ( (!$order_email) && (!$order_phone)) {
				wc_print_notice( __( 'Please check email or phone number', 'woocommerce' ), 'error' );
			} else {
				...
				elseif ( $order && $order->get_id() && $order->get_billing_phone()  === $order_phone  ) {
					do_action( 'woocommerce_track_order', $order->get_id() );
					wc_get_template(
						'order/tracking.php',
						array(
